...
Suppose that $\text{Im}(\omega)>0$ and 
...
\begin{lem}
If $\bu = \nabla^{\perp} H_{0}(\omega r)$ and $p\to 0$ as $r\to \infty$, 
then
\begin{equation}
\lim_{r\to \inf} \sqrt(r) \left(\ff_{1} - i\omega \bu_{1} \right) = 0 \, .
\end{equation}
\end{lem}

\begin{proof}
If $\bu = \nabla^{\perp} H_{0}(\omega r) = (\partial_{y} H_{0}(\omega r), 
-\partial_{x} H_{0}(\omega r))$, then
\begin{equation}
\nabla  p = (\Delta + \omega^2) \bu = (\Delta + \omega^2) \nabla^{\perp} 
H_{0} (\omega r) = 0 \, .
\end{equation}
...
&= 2 \partial_{xy} H_{0}(\omega r) \cdot \frac{x}{r} +
\left[-\partial_{xx} H_{0}(\omega r) + \partial_{yy} H_{0}(\omega r) \right]
\cdot \frac{y}{r} \, .
\end{align}
Based on the mathematica script (radcond1.nb in our mathematica folder), we
get
\begin{equation}
\ff_{1} - i \omega u_{1} = \frac{ky}{r^2} \left( -kr \left(H_{0}(\omega r) - 
i H_{1}(\omega r)\right) +
2 H_{1}(\omega r)\right) \, .
\end{equation}
