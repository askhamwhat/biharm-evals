...
oscillatory Stokeslet, which is the appropriate setting for the Stokes
eigenvalue problem.
...
\cite{Pozrikidis1992} for the problem
\eqref{eq:ostokes}.
...
\begin{equation}
\label{eq:dlformula}
  \cD [\bmu] (\xx) = \oint_\Gamma \left ( \TT_{\cdot,\cdot,\ell}(\bx,\by)
  \nu_\ell(\yy)
...
\begin{equation}
  \cD^{\intercal} [\bmu](\bx) = 
  \oint_\Gamma \left ( \TT_{\cdot,\cdot,\ell}(\bx,\by)\nu_\ell(\xx)
  \right ) \bmu(\yy) \, dS(\yy) \; .
\end{equation}
...
In the following lemma, we review the limiting values of layer 
potentials $\bS$ and $\bD$ on the boundary $\Gamma$.
