...
\begin{equation} \label{eq:doublelayer}
  ...
  \nu_ell(\yy)\right )^\intercal \bmu(\yy) \, dS(\yy) \; ,
\end{equation}
...
\begin{equation}
  \cN [\bmu] = \oint_\Gamma \left ( \TT_{\cdot,\cdot,\ell}\nu_\ell(\xx)
  \right ) \bmu(\yy) \, dS(\yy) \; .
\end{equation}

\begin{remark}
The operators $\cD$ and $\cN$
...
abuse of notation, we will use the symbols $\cD(k)$ and $\cN(k)$ to 
...
\begin{thrm} \label{thrm:rep-theorem}
  ...
  \begin{equation} \label{eq:rep-theorem}
    \bS [\bsigma(\bu,p)](\xx) - \bD[\bu](\xx) = \begin{cases} 
...
\begin{lem}
  ...
  \item If $\int_{\Gamma} p = 0 dS$, then
  $\cW[\bmu \pm 2 \cD^{T}[\bmu]] = 0$,
  \item $\cW[\cS[\bmu]] = 0$,
  \end{itemize}
...
\begin{remark}
Since the pressure for the oscillatory Stokes equation is only 
well-defined up to a constant in either region, we always choose
a normalization for the pressure which satisfies the conditon
$\int_{\Gamma} p dS = 0$.
\end{remark}
