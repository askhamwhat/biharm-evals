...
the term pollution refers to the fact that,
when solving the Helmholtz equation in dimension
greater than one,
the accuracy of the Galerkin solution of
a problem does indeed differ by a constant factor from
the error in the best approximation of the true
solution in the finite element space but this factor
grows as a function of the frequency $k$
...
In constrast, a BIE does not suffer from this
effect.}
...
{\color{red}which are too far from the
  ...
  ellipse $E_\rho$ with $\rho = 1+\sqrt{\epscheb}$.
